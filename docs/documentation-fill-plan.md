...
stale ones. Written 2026-08-31. **Done as of 2026-09-01** (a different
machine picked this up without seeing the plan below, reached the same
Module-not-Workspace call independently, and wrote all 16 files plus the
stale-doc fixes). Left in place as a record rather than deleted.
...
## Tier 1 — Technical docs (contributors) — done

- [x] `docs/adr/0001-use-uno-command-ids.md` — decision record for
...
- [x] `docs/adr/0002-discovery-engine.md` — decision to discover the live
...
- [x] `docs/workspace.md` — reference expanding ADR 0004: `Module` fields, the
      four instances (WRITER/CALC/IMPRESS/DRAW), `module_for_document`, why
      adding an app is "a new Module + templates."
- [x] `docs/coding-standards.md` — from CLAUDE.md's Conventions section plus
...
- [x] `docs/developer-guide.md` — onboarding: repo layout, `pytest`, dev
      tools, the throwaway-headless-instance safety rule (link CLAUDE.md,
      don't repeat it), how to add a new adapter or Module.
- [x] Stale-doc fixes bundled here:
  - [x] `docs/architecture.md` — replace the Workspace/UIElements diagram
        with the actual Module-parameterized flow; Calc/Impress/Draw are
        done, not "future."
  - [x] `docs/project-definition.md` — "Version 1 supports only Writer" ->
        reflect v4.0.0 (all four apps shipped).
  - [x] `docs/roadmap.md` — mark Milestones 0.1-0.7 and Versions 1.0-4.0 done
        (per PROJECT.md); add what's actually next (PROJECT.md's "Next
        Session Tasks").

## Tier 2 — User-facing guides — done

- [x] `docs/glossary.md` — Template, Profile, Module, Apply/Restore,
      non-cumulative, UNO command ID, Discovery, Addon, Sidebar deck,
      `.louim`.
- [x] `docs/teacher-guide.md` — task-oriented: apply a starter template,
      customize via Tools > Customize + "Save Current Layout as
      Template...", share a `.louim` file, restore full menus. No internals.

## Tier 3 — Education essays (`docs/education/`) — done

- [x] `why-progressive-interfaces-matter.md` — pedagogical case for
      progressive disclosure.
- [x] `reducing-cognitive-load.md` — mechanism (fewer visible tools -> less
      decision fatigue), tied to the level-1/level-2/full template pattern.
- [x] `designing-progressive-writer-courses.md` — sequencing level-1 ->
      level-2 -> full across a course.
- [x] `using-louim-in-primary-schools.md` — classroom-specific (young
      learners, simple vocabulary).
- [x] `using-louim-in-adult-education.md` — adult-learner angle (digital
      literacy programs, self-paced).

## Outcome

All three tiers landed in one pass rather than tier-by-tier with review in
between, and a couple of things drifted from this plan along the way: the
first draft leaned heavily on em-dashes, bold, and parenthetical asides
(caught and flattened in a follow-up pass), and `docs/project-definition.md`
below was fixed as part of finishing this plan, not missed.
